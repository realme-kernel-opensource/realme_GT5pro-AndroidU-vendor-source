...
unsigned char F024_Polling_gain(struct cam_ois_ctrl_t *o_ctrl)
{

    unsigned char	u16_dat = 0;

    for(int u16_i = 1; u16_i <= 50; u16_i ++ ) {
        u16_dat = I2C_OIS_8bit__read(o_ctrl,0xF024);
        if((u16_dat&0x01) == 1) {
            break;
        }
        Wait(1000);
...
void WriteOisGainCompensation(struct cam_ois_ctrl_t *o_ctrl)
{

	unsigned short int Gain_X = 0;
	unsigned short int Gain_Y = 0;
	unsigned short int Set_Gain_X = 0;
	unsigned short int Set_Gain_Y = 0;

	Gain_X = I2C_OIS_16bit__read(o_ctrl,0xF07A);//read 0xF07A
	Gain_Y = I2C_OIS_16bit__read(o_ctrl,0xF07C);//read 0xF07C
	CAM_INFO(CAM_OIS, "ccw_test_ois will write  X_gain= 0x%x  Y_gain= 0x%x",Gain_X, Gain_Y);

	Set_Gain_X = (86*Gain_X)/100;
	Set_Gain_Y = (93*Gain_Y)/100;
	CAM_INFO(CAM_OIS, "ccw_test_ois before write Set_Gain_X= 0x%x Set_Gain_Y= 0x%x",Set_Gain_X, Set_Gain_Y);

	I2C_OIS_16bit_write(o_ctrl, 0xF07A, Set_Gain_X);
	F024_Polling_gain(o_ctrl);

	I2C_OIS_16bit_write(o_ctrl, 0xF07C, Set_Gain_Y);
	F024_Polling_gain(o_ctrl);

	Gain_X = I2C_OIS_16bit__read(o_ctrl,0xF07A);//read 0xF07A
	Gain_Y = I2C_OIS_16bit__read(o_ctrl,0xF07C);//read 0xF07C

	CAM_INFO(CAM_OIS, "ccw_test_ois after write  X_gain= 0x%x  Y_gain= 0x%x",Gain_X, Gain_Y);
	return;
}
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		data20 = I2C_OIS_8bit__read(o_ctrl, 0xF020);
		CAM_INFO(CAM_OIS, "ccw_test_log after cam_ois_apply_settings F020:0x%x, count_gain:%d",data20,count_gain);
		if ((data20 == 2) && (count_gain == 1))
		{
			WriteOisGainCompensation(o_ctrl);
			count_gain = 0;
		}
#endif
